...
network=c3nid;else if(document.referrer)
{var isInIFrame=window.location!==window.parent.location,sUrl;if(isInIFrame)
...
var iIndex;if(sUrl)
{nLink.href=sUrl;if(nLink.search)
{var queryString=decodeURI(nLink.search.slice(1)),aParameters=queryString.split('&'),aNidLabels=c3nidlabels.split(','),ga_values={},ga_value_count=0,nid_values={api:'',cid:'',c3_label:{channel:'',nid:''},ga_nid:'',reserved_nid:{channel:'',nid:''},xnid:{channel:'',nid:''},xnid_label:{channel:'',nid:''}},nid='';for(var i=0;i<aParameters.length;i++)
{var aParamSplit=aParameters[i].split('='),sKey=aParamSplit[0].toLowerCase(),sValue=aParamSplit[1];if(sValue){if(xnidLabels[sKey])
{nid_values.xnid_label.channel=xnidLabels[sKey].c3ch;if(sValue&&xnidLabels[sKey].c3nid==='%param%'){nid_values.xnid_label.nid=sValue}
else nid_values.xnid_label.nid=xnidLabels[sKey].c3nid}
if(typeof c3nidlabelaliases[sKey]!=='undefined')
{sKey=c3nidlabelaliases[sKey]}
if(indexOf(sKey,oValidClientParamKeys)!==-1){clientParams[sKey]=sValue}
if(indexOf(sKey,oValidApiParamKeys)!==-1)
{if(sValue)
nid_values.api=sKey+':'+sValue}
if(typeof reservedNids[sKey]!=='undefined')
{var reservedNid=reservedNids[sKey];nid_values.reserved_nid.channel='Uncategorized';if(sValue)
...
ctdataString+='&c3ps='+c3ps;if(refBlocked){ctdataString+='&url='+window.location;}if(c3ip)
...
var nidlbl={},oClientParams=[],xnidLabels={},reservedNids={},c3Labels={},gaLabels={},labelPriority=[]
    ...
    c3CTJScall(true,document.title,30,'569','1370768964310046208','','//569-ct.c3tag.com/ctv4/ctcall.php','Mac OSX Puma',"'c3','utm_source','utm_medium','utm_campaign','cvosrc'",nidlbl, oClientParams,xnidLabels,reservedNids,'1','','','','');
})(this);
