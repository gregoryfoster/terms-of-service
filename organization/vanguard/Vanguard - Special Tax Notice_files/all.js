...
vg.Layer.getGlobalHeaderElement=function()
{
"use strict";
return document.getElementById("gh")||
document.getElementById("vgc-globalHeader")||
document.getElementById("vgn-globalHeader")||
document.getElementById("ghNoNav");
};
vg.Layer.getGlobalFooterElement=function()
{
"use strict";
return document.getElementById("gf")||
document.getElementById("vgc-globalFooter")||
document.getElementById("sFooter")||
vg.html.getElements(document.getElementById("body"),{tagName:'div'}, function(node){return vg.html.hasStyle("gblFoot", node);})[0];
};
...
{
vg.html.addEventListener(wrapDiv, 'scroll', function(e){
controller.onPageScroll(e);
});
}
...
vg.html.addEventListener(this.viewNode, 'keydown', function(e)
{
controller.keyEventHandler(e);
});
if(this.scrollDiv)
{
vg.html.addEventListener(this.scrollDiv, 'scroll', function(e){
jsCBDcloseSelectOneMenu(e);
});
}
if(cbd.page.isResponsiveCapable===true)
{
vg.html.addEventListener(window, vg.event.BROWSER_RESIZE_END,
function(e){
controller.adjustOpenRespLayer();
});
}
},
...
controller.ddLoaded=true;
controller.makeMoveable();
});
}
if(this.resizeable )
{
cbd.loader.addCallback(function(){
controller.makeResizeable();
});
}
...
if(key==vg.constants.KEY_CODES.TAB)
{
...
if((targ==this.firstElem||targ===this.viewNode)&&e.shiftKey)
{
...
else if(key===vg.constants.KEY_CODES.ESC)
{
this._handleEscKey(targ);
vg.html.preventDefault(e);
}
else if((key===vg.constants.KEY_CODES.DOWN_ARROW||key===vg.constants.KEY_CODES.UP_ARROW||
key===vg.constants.KEY_CODES.LEFT_ARROW||key===vg.constants.KEY_CODES.RIGHT_ARROW)&&
!vg.html.isAncestor(this.scrollDiv,document.activeElement)&&
this.scrollDiv)
{
this.scrollDiv.tabIndex=-1;
jsCBDSetFocus(this.scrollDiv);
vg.html.preventDefault(e);
}
},
_handleEscKey:function(target)
{
"use strict";
var closeButtonElm=document.getElementById(this.id+"_closeButton");
if(closeButtonElm)
{
closeButtonElm.click();
}
else if(target&&("errFormLayer"===target.id||"errFormLayer_butInput"===target.id))
{
vg.validation.closeErrLayer("errFormLayer");
_cbdCloseErrLayerCallback();
}
},
geoChanged:function()
{
...
if(this.id!=="errFormLayer"){
jsCBDSetAriaLiveContent('Layer opened',true);
}
},
...
controller.setInitialFocusOnLayer();
...
if(this.id!=="errFormLayer"){
jsCBDSetAriaLiveContent('Layer closed',true);
}
},
...
id=vg.html.getElements(this.viewNode,{
tagName:'div',
attrName:'class'
}, function(elem){
return vg.html.hasStyle('rd1', elem);
})[0];
if(id)
{
...
vg.smil.animateElement(id, 'opacity', this.fadeDuration, 0, 0, 1, null,
function(){
vg.util._removeIEfilterProp(controller.viewNode);
});
if(ieQuirksMode)
{
_cbdCheckConditionsAndExecute(function(){
return controller.viewNode.style.filter!='';
}, function(){
jsCBDtoggleElement(controller.viewNode, true);
});
}
else
{
...
"use strict";
var controller=this;
vg.smil.animateElement(this.viewNode.getAttribute('id'), 'opacity',
1000, 0, 1, 0, null, function(){
controller.onCloseAnimationEnd();
});
},
...
controller=this;
vg.smil.animateElement(rbTable.getAttribute('id'), attributes,
this.transitionDuration, 0, null, null, null, function(){
controller.onCloseAnimationEnd();
});
},
...
var controller=this;
vg.smil.animateElement(rbTable.getAttribute('id'), attributes,
this.transitionDuration, 250, null, null, null, function(){
controller.showLeader();
}, null);
},
showLeader:function()
{
...
setInitialFocusOnLayer:function()
{
"use strict";
jsCBDSetFocus(document.getElementById(this.id));
},
setFocus:function()
{
...
{
var controller=this;
vg.html.addEventListener(window, 'resize', function(e){
controller.setHeightOfScrollDiv();
}, "layerNGwindowResize-"+controller.viewNode.id);
}
};
...
scrollNoScrollWidth=vg.html.getObjWidth(this.scroll_noscroll_container ),
noScrollWidth,
windowWidth,
headerWidth;
this.adjustWidthForScrollBars();
noScrollWidth=this.noScrollDiv?vg.html.getObjWidth(this.noScrollDiv ):0;
if(scrollNoScrollWidth < noScrollWidth )
{
scrollNoScrollWidth=noScrollWidth;
windowWidth=this.outerDiv.firstChild.offsetLeft*2+scrollNoScrollWidth+this.paddingLeft+this.paddingRight;
this.viewNode.style.width=windowWidth+"px";
isAdjusted=true;
}
headerWidth=(this.headerArea!=null)?vg.html.getObjWidth(this.headerArea ) - this.paddingLeft - this.paddingRight:0;
...
vg.html.addEventListener(window, "popstate", function(e){
THIS.onPopState(e.state);
}, vg.Layer.HISTORY_POPSTATE+this.id);
}
};
...
var toggleElementFunc=undefined;
if(!jsCBDisVisibleElement(this.viewNode))
{
...
SelectOneMenu.PULLDOWN_HIGHLIGHT="vg-SelOneMenuHover vg-SelOneMenuHoverText";
SelectOneMenu.PULLDOWN_TEXT_FOCUS='vg-SelOneMenuFocusText vg-SelOneMenuHover';
...
SelectOneMenu.KEY_CODES=vg.constants.KEY_CODES;
...
if(controller.openOnHover&&!isTouchScreen)
{
controller.tID=setTimeout(function(){
controller._hideMenu()
}, 0);
}
}, "SOM_COMP_MOUSEOUT_TOUCHEND" );
vg.html.addEventListener(document.body, vg.event.GSTR_DOWNSTART,
function(e){
jsCBDcloseSelectOneMenu(e)
}, "SOM_BODY_DOWNSTART");
...
vg.html.addEventListener(this.viewNode, vg.event.GSTR_CLICKEND,
function(e){
controller._clickEvent(e)
}, "SOM_COMP_TOUCHCLICK");
...
vg.html.addEventListener(this.menu, 'focus', function(e){
controller._focus()
});
vg.html.addEventListener(this.menu, 'keydown', function(e){
controller._menuKeyEvent(e)
});
if(nextGen)
{
...
vg.html.addEventListener(this.aTag, 'keydown', function(e){
controller._menuKeyEvent(e)
});
}
else
{
vg.html.addEventListener(this.borderDiv, 'focus', function(e){
controller._focus(e)
});
vg.html.addEventListener(this.borderDiv, 'keydown', function(e){
controller._menuKeyEvent(e)
});
vg.html.addEventListener(this.borderDiv, 'blur', function(){
controller._blur()
});
}
if(!isTouchScreen)
...
clearTimeout(controller.tID);
controller._showMenu();
controller._highlightItem(e, true);
}
);
}
...
{
this.borderDiv.focus();
}
},
_highlightItem:function(e, manuallyFocus)
{
if(!e)
{
return;
...
cell=eNode.firstChild;
if(this.selectedItem&&this.selectedItem!==this.highlightedItem)
{
...
{
return node.getAttribute('disabled' )||(node.getAttribute('aria-disabled'))==='true';
},
...
if(!this.initLoad )
{
this._highlightItem(eNode, true);
}
...
if(this.highlightedItem)
{
this.highlightedItem.firstChild.setAttribute('aria-selected', true);
...
_focus:function(event, skipFocusEvent)
{
...
if(!skipFocusEvent&&onFocusJs!==null)
{
vg.comp._execEventHandler(onFocusJs, this.viewNode);
}
},
_blur:function(event, skipBlurEvent)
{
var onBlurJs=this.viewNode.getAttribute('onBlurJs');
if(!this.menuOpen)
{
jsCBDdeleteStyle(this.contDiv, SelectOneMenu.PULLDOWN_TEXT_FOCUS);
if(!skipBlurEvent&&onBlurJs!==null)
{
vg.comp._execEventHandler(onBlurJs, this.viewNode);
}
}
},
_execOntoggle:function(){
var onToggleJs=this.viewNode.getAttribute('ontoggle'),
isHidden=!this.menuOpen,
list, myFunc,
trigger=cbd.page.isNextGen?this.aTag:this.borderDiv;
if(onToggleJs!=null)
{
list=onToggleJs.split(',');
if(onToggleJs.toLowerCase().indexOf('bleedthru') >=0)
{
myFunc=onToggleJs.slice(0, onToggleJs.indexOf(')'));
if(list.length===3)
{
onToggleJs=myFunc+','+'null,'+isHidden+')';
}
if(list.length===4)
{
onToggleJs=myFunc+','+isHidden+')';
}
}
eval(onToggleJs);
}
...
var event=jsCBDgetEvent(e),
key=jsCBDgetKey(event),
keyCodes=SelectOneMenu.KEY_CODES,
moveForward=key===keyCodes.DOWN_ARROW||key===keyCodes.RIGHT_ARROW||(key===keyCodes.TAB&&!event.shiftKey),
moveBackward=key===keyCodes.UP_ARROW||key===keyCodes.LEFT_ARROW||(key===keyCodes.TAB&&event.shiftKey),
direction=this.highlightIndex+1 < this.options.length&&moveForward?'forward':
(this.highlightIndex - 1 >=0&&moveBackward?'backward':undefined),
previouslyHighlightedIndex=this.highlightIndex,
isDisabled=this.disabled||vg.html.getDisabledAttr(this.viewNode),
heightPerOpt, optionY, firstChild;
isDisabled=isDisabled===true||isDisabled==='true'||isDisabled==='disabled';
if(key===keyCodes.UP_ARROW||key===keyCodes.DOWN_ARROW||key===keyCodes.LEFT_ARROW||key===keyCodes.RIGHT_ARROW||key===keyCodes.SPACE)
{
vg.html.preventDefault(e);
}
if(isDisabled)
...
if(direction)
{
this._getNextSelectableItem(key, direction);
}
if(this.menuOpen&&(moveBackward||moveForward)&&previouslyHighlightedIndex!==this.highlightIndex)
{
this._highlightItem(this.options[this.highlightIndex], true);
if(vg.html._hasVerticalScrollBar(this.scrollDiv))
{
heightPerOpt=this.scrollDiv.scrollHeight/this.options.length;
optionY=(heightPerOpt*this.highlightIndex)+vg.html.getObjY(this.scrollDiv);
...
{
firstChild=vg.html.getFirstChild(this.options[this.highlightIndex]);
vg.html.preventDefault(e);
if(this._selectItem(firstChild, false, false))
{
firstChild.click();
this.focus();
}
}
else if(this.menuOpen&&(key===keyCodes.UP_ARROW||key===keyCodes.ESC||key===keyCodes.TAB))
{
this._hideMenu();
this.focus();
if(event.shiftKey)
...
if(this.menuOpen&&this.highlightedItem)
{
this.highlightedItem.firstChild.focus();
}
}
},
_getNextSelectableItem:function(key, direction){
if(!direction)
{
return;
}
var indexChange=(direction==='forward'?1:-1),
startVal=this.highlightIndex+indexChange,
numOptions=this.options.length,
currNode;
for(var x=startVal;x >=0&&x < numOptions;x+=indexChange)
{
currNode=vg.html.getFirstChild(this.options[x]);
if(!this._isUnselectable(currNode))
{
...
{
var item, itemValue, label, searchStartPos, shouldSkip,
matchFound;
if(this.lastSortKeyValue!=keyChar.toUpperCase())
{
this._resetSorting(keyChar.toUpperCase())
}
searchStartPos=(this.lastSortKeyIndex > -1)?this.lastSortKeyIndex+1:0;
for(var i=searchStartPos;i < this.options.length;i++)
{
item=vg.html.getFirstChild(this.options[i]);
shouldSkip=this._isDisabled(item);
shouldSkip=!this.menuOpen&&(this._isUnselectable(item)||shouldSkip==='true'||shouldSkip===true||shouldSkip==='disabled');
itemValue=item.getAttribute('value');
label=item.innerHTML.toUpperCase();
if(vg.string.startsWith(label, keyChar.toUpperCase())&&itemValue!=='none'&&!shouldSkip)
{
this.highlightIndex=item.index;
this.lastSortKeyIndex=item.index;
this._highlightItem(item, false);
matchFound=true;
break;
}
}
...
var key=jsCBDgetKey(jsCBDgetEvent(e)),
keyCodes=SelectOneMenu.KEY_CODES,
isValidEntryKey=key===keyCodes.DOWN_ARROW||key===keyCodes.ENTER||key===keyCodes.SPACE,
isDisabled=this.disabled||vg.html.getDisabledAttr(this.viewNode);
isDisabled=isDisabled===true||isDisabled==='true'||isDisabled==='disabled';
if(isValidEntryKey&&!this.menuOpen&&!isDisabled)
{
this._showMenu();
vg.html.preventDefault(e);
vg.html.stopImmediateEventPropagation(e);
}
},
_clickEvent:function(e)
{
var disabled, closeOnClick;
...
{
closeOnClick=(this.openOnHover&&!cbd.browser.isTouchScreen)?false:true;
...
_hideMenu:function()
{
jsCBDdeleteStyle(this.labelDiv, SelectOneMenu.PULLDOWN_TEXT_FOCUS);
this.menuOpen=false;
...
this._highlightItem(this.options[this.selectedIndex], false);
if(!isMediaQuerySizeLessThanLarge)
{
...
jsCBDaddStyle(this.labelDiv, SelectOneMenu.PULLDOWN_TEXT_FOCUS);
...
contr._highlightItem(e, true);
},
_selectItem=function(e){
contr._selectItem(e, false, false);
contr.focus();
};
for(var i=0;i < this.options.length;i++)
{
var optionItem=vg.html.getFirstChild(this.options[i]);
vg.html.addEventListener(optionItem, 'mouseover', _highlightItem);
vg.html.addEventListener(optionItem, 'click', _selectItem);
}
...
{
var pullDown,onBlurJs;
if(SelectOneMenu.selectedPulldownId!=null)
{
pullDown=document.getElementById(SelectOneMenu.selectedPulldownId);
if(pullDown!=null)
{
...
if(controller.changeValue&&controller.changeValue!==controller.valueInput.value)
{
controller._executeOnChangeJs(true);
}
onBlurJs=controller.viewNode.getAttribute('onBlurJs');
if(onBlurJs!==null)
{
...
{
vg.html.addStyle(SelectOneMenu.PULLDOWN_TEXT_FOCUS, this.contDiv );
...
{
vg.html.removeStyle(SelectOneMenu.PULLDOWN_TEXT_FOCUS, this.contDiv );
...
vg.html.addEventListener(this.link, 'click', function(e){
parentController.select(e);
});
}
...
{
return vg.html.getElements(this.viewNode,{
tagName:'img',
attrName:'class'
}, function(node){
return vg.html.hasStyle('slidertab-arrow', node);
})[0];
},
_getLink:function()
{
...
var controller=this;
vg.html.addEventListener(this.addButton, 'click', function(e){
controller.add(e);
});
vg.html.addEventListener(this.removeButton, 'click', function(e){
controller.remove(e);
});
if(this.addAllButton){
vg.html.addEventListener(this.addAllButton, 'click', function(e){
controller.addAll(e);
});
}
if(this.removeAllButton){
vg.html.addEventListener(this.removeAllButton, 'click', function(e){
controller.removeAll(e);
});
}
}
...
if(this.sourceList.options[idx].value=="pad"){
return false;
}
...
this.dateCodes=this._getParam(options['codedDates'],[]);
...
vgHtmlNS.addEventListener(document.body, "touchstart" , function(event)
{
...
{
this.leaderId;
this.content_css=window.isNextGen?'infoBoxContent':'roundBoxCtnt';
...
if(e&&(e.type==='focus'||e.type==='keyup'))
{
this.open(e);
}
},
...
!vg.html.hasStyle(InfoBox.CLASSES.A11Y, this.globalInfoBox))
{
return true;
}
return false;
...
vg.html.addEventListener(this.viewNode,
vg.event.GSTR_HOVERSTART, function(e){
controller._hoverStart(e);
}, "INFOBOX_HOVERSTART");
if(cbd.browser.isTouchScreen)
{
vg.html.removeEventListenerById(this.viewNode, "INFOBOX_HOVEREND");
vg.html.addEventListener(this.viewNode,
vg.event.GSTR_HOVEREND, function(e){
controller._hoverEnd(e);
}, "INFOBOX_HOVEREND");
if(cbd.page.isResponsiveCapable)
{
...
if(controller&&!vg.html.hasStyle(InfoBox.CLASSES.A11Y, controller.globalInfoBox))
{
controller.close();
setTimeout(function(){
if(controller)
...
vg.html.addEventListener(this.viewNode, 'keydown',
function(e){
controller._keyDown(e);
}, "INFOBOX_KEYDOWN");
vg.html.removeEventListenerById(this.targetNode, "INFOBOX_BLUR");
vg.html.addEventListener(this.targetNode, 'blur', function(
e){
controller._blur(e);
}, "INFOBOX_BLUR");
}
...
vg.html.addEventListener(this.viewNode, closeEventType,
function(e){
controller.close(e);
}, "INFOBOX_CLOSE");
}
...
vg.html.addEventListener(this.globalInfoBox,
vg.event.GSTR_HOVERSTART, function(e){
controller._hoverStartGlobalInfoBox(e);
}, "INFOBOX_HOVERSTART");
vg.html.removeEventListenerById(this.globalInfoBox, "INFOBOX_HOVEREND");
vg.html.addEventListener(this.globalInfoBox,
vg.event.GSTR_HOVEREND, function(e){
controller._hoverEndGlobalInfoBox(e);
}, "INFOBOX_HOVEREND");
}
},
...
_blur:function(e)
{
if(!vg.html.hasStyle(InfoBox.CLASSES.A11Y, this.globalInfoBox))
{
return;
}
else
...
keyCodes=InfoBox.KEY_CODES,
isVisible=!vg.html.hasStyle(InfoBox.CLASSES.A11Y, this.globalInfoBox);
if((event.shiftKey&&key===keyCodes.TAB)||(key===keyCodes.ESC&&isVisible))
{
this.close(event, true);
}
else if(key===keyCodes.DOWN_ARROW||key===keyCodes.RIGHT_ARROW)
{
if(isVisible&&!this.disableDefaultFocus)
{
jsCBDSetFocus(this.globalInfoBox);
}
else
{
this.open(eventInfo);
...
else if(key===keyCodes.TAB&&isVisible&&eventInfo.target===this.targetNode&&!this.disableDefaultFocus)
{
jsCBDSetFocus(this.globalInfoBox);
...
{
var trueEvent=jsCBDgetEvent(event),
eventPos=vg.event.getPosition(trueEvent);
return{event:trueEvent, eventPos:eventPos, target:trueEvent.target, keyCode:jsCBDgetKey(trueEvent)};
},
...
if(!preventOpen)
{
vg.html.removeStyle(InfoBox.CLASSES.A11Y, globalInfoBox);
globalInfoBox.setAttribute(ARIA.HIDDEN, false);
if(Populator.exists(this.viewNode))
{
Populator._copy(this.viewNode, this.contentNode);
Populator._execute(this.contentNode, function(){
...
vg.html.addEventListener(globalInfoBox.leader,
vg.event.GSTR_HOVERSTART, function(event){
controller._hoverStartGlobalInfoBox(event);
}, "INFOBOX_HOVERSTART");
vg.html.removeEventListenerById(globalInfoBox.leader, "INFOBOX_HOVEREND");
vg.html.addEventListener(globalInfoBox.leader,
vg.event.GSTR_HOVEREND, function(event){
controller._hoverEndGlobalInfoBox(event);
}, "INFOBOX_HOVEREND");
}
...
vg.html.removeEventListenerById(globalInfoBox, 'INFOBOX_KEYDOWN_ALL');
vg.html.addEventListener(globalInfoBox, 'keydown', function(e){
controller.keyEventHandler(e);
}, 'INFOBOX_KEYDOWN_ALL');
this.setFirstAndLastInputElements(false);
this.targetNode.setAttribute(ARIA.EXPANDED, true);
}
},
close:function(e, doNotDelay)
{
this.globalInfoBox.onlink=false;
this.mouseIsOverLink=false;
if(this.iBoxPersist&&!doNotDelay)
{
this.callCheckClose();
return;
}
...
if(key===keyCodes.TAB)
{
if(target===this.globalInfoBox)
{
focusTarget=(!shifted&&this.firstElem)||this.targetNode;
}
else if(target===this.firstElem&&shifted)
{
focusTarget=this.globalInfoBox;
}
else if(target===this.lastElem&&!shifted)
{
focusTarget=this.targetNode;
}
jsCBDSetFocus(focusTarget);
if(focusTarget===this.targetNode)
{
if(shifted)
{
vg.html.preventDefault(e);
...
InfoBox.KEY_CODES=vg.constants.KEY_CODES;
InfoBox.CLASSES={A11Y:'accessibleOffscreenText'};
...
InfoBox.close=function(event)
{
if(InfoBoxFactory==null||(event&&(vg.html.isAncestor(InfoBoxFactory.globalInfoBox, event.target)||event.target===InfoBoxFactory.targetNode)))
{
return;
}
...
{
vg.html.addStyle(InfoBox.CLASSES.A11Y, InfoBoxFactory.globalInfoBox);
}
...
InfoBoxFactory.targetNode.setAttribute(ARIA.EXPANDED, false);
InfoBoxFactory.globalInfoBox.setAttribute(ARIA.HIDDEN, true);
jsCBDgetElementsByAttr(InfoBoxFactory.globalInfoBox, "div", "className", InfoBoxFactory.content_css)[0].innerHTML='';
};
...
InfoBox.moveConfig=function(compId, targetId, ariaConfig)
{
...
{
target.setAttribute(prop, config[prop]);
}
}
target.setAttribute('tabindex', target.getAttribute('tabindex')||0);
vg.html.addEventListener(target, 'mouseover', function(e){
compManager._setCompConfig(compId, 'INFOBOX', e);
});
vg.html.addEventListener(target, 'keyup', function(e){
if(e.keyCode===InfoBox.KEY_CODES.TAB)
{
compManager._setCompConfig(compId, 'INFOBOX', e);
}
});
...
if(listeningNode)
{
vgHtml.addEventListener(listeningNode, vgEvent.GSTR_HOVERSTART,
function(){
THIS.hover();
});
vgHtml.addEventListener(listeningNode, 'focus', function()
{
THIS.hover();
});
vgHtml.addEventListener(listeningNode, 'blur', function(e)
{
THIS.nohover(e);
});
if(cbd.browser.isTouchScreen)
{
vgHtml.addEventListener(listeningNode, 'click', function(e){
THIS.nohover(e);
});
vgHtml.addEventListener(listeningNode, vgEvent.GSTR_MOVE, function(
e){
THIS._hoverTouchMove(e);
});
}
else
{
vgHtml.addEventListener(listeningNode, 'click', function(){
THIS.click(false);
});
vgHtml.addEventListener(listeningNode, 'keydown', function(e){
THIS.keyEventHandler(e);
});
vgHtml.addEventListener(listeningNode, vgEvent.GSTR_HOVEREND,
function(e){
THIS.nohover(e);
});
}
...
var THIS=this;
Populator._execute(node1, function(){
THIS.contentLoaded();
});
}
...
listeningNode=_getAriaControlNode(this);
if(listeningNode){
listeningNode.setAttribute('aria-expanded', this.openflag);
}
};
_getAriaControlNode=function(navBox)
{
"use strict";
var controlNode,
navOpenedBy;
if(navBox&&navBox instanceof vg.NavBox)
{
controlNode=navBox.getListeningNode();
if(!controlNode)
{
navOpenedBy=navBox.openedBy;
controlNode=_getNoHeaderNavCallerNode(navOpenedBy);
}
}
return controlNode;
};
_getNoHeaderNavCallerNode=function(navOpenedBy)
{
"use strict";
var controlNode;
if(navOpenedBy)
{
if(typeof navOpenedBy==='string')
{
controlNode=document.getElementById(navOpenedBy);
}
else if(typeof navOpenedBy==='object')
{
controlNode=navOpenedBy;
}
}
return controlNode;
};
...
{
this.anim=vg.smil.animateElement(id+'_body', 'height', time, 0,
posTop, posBot, null, function NavBox_animFinished(){
THIS.animationFinished();
}, null, null, null, null, THIS.easing,
this.disableGeoChangedEvent);
}
};
...
this.anim=vg.smil
.animateElement(id+'_top', attributes, time, 250, null, null,
null, function NavBox_animFinished(){
THIS.animationFinished();
}, null, null, null, null, THIS.easing,
this.disableGeoChangedEvent);
};
vg.NavBox.prototype.keyEventHandler=function(event )
{
"use strict";
...
var controller=this;
vg.html.addEventListener(this.viewNode, 'keyup', function(e){
controller._keyup(e);
});
...
ariaDescribedByID=this.viewNode.getAttribute('id')+'_ariaText ',
ariaDescribedby;
for(i=0;i < itemsLength;i++)
{
jsCBDtoggleLink(this.items[i].viewNode, true, "current");
this.items[i].selected=false;
this.items[i].viewNode.setAttribute(vg.constants.ARIA.SELECTED, false);
this.items[i].viewNode.setAttribute(vg.constants.ARIA.DESCRIBEDBY, this.items[i].viewNode.getAttribute(vg.constants.ARIA.DESCRIBEDBY).replace(ariaDescribedByID, ''));
}
if(!this.allSelected)
{
jsCBDtoggleLink({link:this.items[selectedNode.index].viewNode, state:false, disabledClass:"current", suppressDisableAttr:true});
this.items[selectedNode.index].viewNode.setAttribute(vg.constants.ARIA.SELECTED, true);
this.items[selectedNode.index].viewNode.setAttribute("tabindex", "0");
ariaDescribedby=this.items[selectedNode.index].viewNode.getAttribute(vg.constants.ARIA.DESCRIBEDBY);
this.items[selectedNode.index].viewNode.setAttribute(vg.constants.ARIA.DESCRIBEDBY, ariaDescribedByID+ariaDescribedby);
}
selectedNode.selected=true;
this.selectedIndex=selectedNode.index;
this.viewNode.setAttribute('selecteditem',selectedNode.index);
this.viewNode.setAttribute(vg.constants.ARIA.SELECTED, true);
this.viewNode.setAttribute('tabindex', "0");
...
var key=jsCBDgetKey(jsCBDgetEvent(e)),
i;
if(key===vg.constants.KEY_CODES.TAB&&e.target===this.viewNode&&!e.shiftKey)
{
for(i=0;i < this.items.length;i++)
{
if(this.items[i].viewNode.getAttribute('class')==="current")
{
jsCBDSetFocus(this.items[i].viewNode);
vg.html.preventDefault(e);
break;
}
else
{
jsCBDSetFocus(this.items[0].viewNode);
}
}
}
},
...
controller.selectLinkItem(e);
});
vg.html.addEventListener(this.viewNode.parentElement, 'keydown',
function(e){
controller._keydown(e);
});
...
{
jsCBDSetAriaLiveContent('selected', false);
...
else if(key===vg.constants.KEY_CODES.SPACE&&!disabled)
{
this.viewNode.click();
}
},
...
{
var key=jsCBDgetKey(jsCBDgetEvent(e));
if(key===vg.constants.KEY_CODES.TAB&&e.shiftKey&&e.target===this.parentController.items[0].viewNode)
{
jsCBDSetFocus(this.viewNode.parentElement.parentElement);
}
if(key===vg.constants.KEY_CODES.SPACE)
{
this.selectLinkItem(e);
vg.html.preventDefault(e);
}
}
};
...
this.items=vg.html.getElements(this.viewNode,{tagName:'a', attrName:'role', attrValue:'option'});
};
...
{
"use strict";
var THIS=this, isLargeScreen=cbdcommon.screen.isMediaQuerySizeLarge(),  attributes, afterAnim,
transDur=400;
if(!this.hasShowAsDropDown()&&this.dropdownOpen)
{
transDur=1;
...
{
if(this.items[i].getAttribute('aria-selected')==='true')
{
jsCBDSetFocus(this.items[i]);
vg.html.preventDefault(e);
break;
}
else
{
jsCBDSetFocus(this.items[0]);
}
}
}
},
...
{
var key=jsCBDgetKey(jsCBDgetEvent(e));
if(key===vg.constants.KEY_CODES.TAB&&e.shiftKey&&e.target===this.items[0])
{
jsCBDSetFocus(this.viewNode);
}
if(key===vg.constants.KEY_CODES.SPACE||key===vg.constants.KEY_CODES.ENTER)
{
e.target.click();
jsCBDSetAriaLiveContent('selected', false);
vg.html.preventDefault(e);
...
if(this.triggerSpot=='ALERT_BOX'){
vg.html.addEventListener(this.alertBox, 'click', function(e)
{
THIS._expand();
});
}
...
{
vg.html.addEventListener(this.alertTitleClosed, 'click', function(e)
{
THIS._expand();
});
vg.html.addEventListener(this.alertTitleOpen, 'click', function(e)
{
THIS._expand();
});
}
...
var THIS=this;
var afterAnimFunc=function()
{
THIS._postAnimation(state );
};
vg.smil.animateElement(this.alertContainer, attributes, 250, 0, null, null, null, afterAnimFunc , null);
}
...
this.items[i].viewNode.setAttribute('aria-selected', false);
this.items[i].viewNode.setAttribute('tabindex', '-1');
}
}
vg.html.replaceClass(this.items[selectedIndex].viewNode, "unselected", "current" );
this.items[selectedIndex].inState=true;
this.currentItem.viewNode.setAttribute('aria-selected', true);
this.currentItem.viewNode.setAttribute('tabindex', '0');
...
if(!item.header)
{
iHeight=(firefox)?vg.html.getObjHeight(vg.html.getFirstChild(item.stTab)):vg.html.getObjHeight(item.viewNode);
...
var controller=this;
vg.html.addEventListener(this.viewNode, 'click', function(e){
controller.selectedSideTab(e)
});
vg.html.addEventListener(this.viewNode, 'keydown', function(e){
controller.tabkeyEventHandler(e)
});
this.parentController=vg.comp.findController(this.viewNode, true);
this.parentController.addItem(this);
}
...
tabkeyEventHandler:function(e)
{
var count=0;
index=this.index,
nextTab=jsCBDgetEvent(e).target.nextElementSibling,
key=jsCBDgetKey(jsCBDgetEvent(e)),
length=this.parentController.items.length,
moveForward=key===SideTab.KEY_CODES.RIGHT_ARROW||key===SideTab.KEY_CODES.DOWN_ARROW,
arrowKeys=moveForward||key===SideTab.KEY_CODES.UP_ARROW||key===SideTab.KEY_CODES.LEFT_ARROW,
...
if(key===SideTab.KEY_CODES.SPACE_BAR||key===SideTab.KEY_CODES.ENTER)
{
this.selectedSideTab(e);
vg.html.preventDefault(e);
}
if(arrowKeys)
{
index=getIndex(index);
nextTab=this.parentController.items[index].viewNode;
while(cbd.adapter.hasClass(nextTab,"stHeader")&&count <(length - 1))
{
index=getIndex(index);
nextTab=this.parentController.items[index].viewNode;
count++;
}
jsCBDSetFocus(nextTab);
vg.html.preventDefault(e);
}
}
}
...
controller.onColRowHeader(e);
};
for(index=0;index < 9;index++)
{
vg.html.addEventListener(this.items[index], 'mousedown', onItem);
vg.html.addEventListener(this.items[index], 'mouseover', onItem);
vg.html.addEventListener(this.items[index], 'mouseout', onItem);
vg.html.addEventListener(this.items[index], 'click', onItem);
}
for(index=0;index < 6;index++)
{
vg.html.addEventListener(this.headers[index], 'mousedown', onColRowHeader);
vg.html.addEventListener(this.headers[index], 'mouseover', onColRowHeader);
vg.html.addEventListener(this.headers[index], 'mouseout', onColRowHeader);
vg.html.addEventListener(this.headers[index], 'click', onColRowHeader);
}
};
...
if(state=="mouseover"&&this.allSelected){
state="mouseoverAll";
}
if(state=="mouseout"&&this.allSelected){
state="mouseoutAll";
}
if(state=="mousedown"&&this.allSelected){
state="mousedownAll";
}
...
for(index=0;index < itemsArr.length;index++)
{
itemNo=parseInt(itemsArr[index])-1;
if(itemNo >=0&&itemNo <=8){
this.setItemState(itemNo,true);
}
}
...
itemNo=parseInt(itemsArr[index])-1;
if(itemNo >=0&&itemNo <=8){
this.setItemState(itemNo,false);
}
}
this.allSelected=false;
this.setHiddenInputValue();
