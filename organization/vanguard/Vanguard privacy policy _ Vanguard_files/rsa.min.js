...
ProxyCollector.doAjax=function(c,b){var d=document.location.protocol+"//"+c+":"+getRandomPort()+"/NonExistentImage"+getRandomPort()+".gif";
var a;
var f;
if(window.XDomainRequest){a=new window.XDomainRequest();
f=new Timer();
try{a.timeout=ProxyCollector.PROXY_DETECTION_TIMEOUT;
a.ontimeout=function(){b.call(this,-1);
a.abort()
};
a.onprogress=function(){b.call(this,f.duration());
a.abort()
};
a.onerror=function(){b.call(this,f.duration());
a.abort()
};
a.open("GET",d,true);
a.send()
}catch(e){ProxyCollector.doAjaxViaImage(b,d)
}}else{a=new XMLHttpRequest();
var g="ontimeout" in a;
f=new Timer();
try{a.onreadystatechange=function(){if(a.readyState==4){if(((typeof ProxyCollector.internalPingTime==="undefined")&&((new RegExp("internalPingTime")).test(b)))||((typeof ProxyCollector.externalPingTime==="undefined")&&((new RegExp("externalPingTime")).test(b)))){b.call(this,f.duration())
}}};
a.timeout=ProxyCollector.PROXY_DETECTION_TIMEOUT;
a.ontimeout=function(){b.call(this,-1);
a.abort()
};
a.open("GET",d,true);
a.send();
if(!g){ProxyCollector.isTimedOut(b,ProxyCollector.PROXY_DETECTION_TIMEOUT-f.duration(),a)
}}catch(e){ProxyCollector.doAjaxViaImage(b,d)
}}};
...
var PAIR="=";
var DEV="~";
function FingerPrint(){var a="3.4.2.0_1";
var b=new Hashtable();
b.put("npnul32","def");
...
var L=Z.collection_status;
var ab=this.getConfig().collection_mode=="partial";
var V=this.getConfig().output_size_limit;
var G=M.length;
var N="@";
var Y=";";
var ad=",";
...
for(var O=P.size();
O>0;
O--){var aa=P.getByIndex(O);
var S=aa.serialize()+Y;
q("elementsStr.length: "+H.length);
if(ab&&((U+H.length+S.length)>V)){b=true;
break
}var I=aa.type().match("INPUT:checkbox");
if(I==null){if(aa.length()>0&&aa.eventCount()==0){q("collecting element without input: "+aa);
H=S+H
}}}U+=H.length;
var D="";
...
var Q=R.index();
var F=R.serialize()+Y;
var S=P.getByIndex(Q).serialize()+Y;
var C=F.length;
if(!K[Q]){C+=S.length
}q("eventsStr.length: "+D.length);
if(ab&&((U+D.length+C)>V)){b=true;
break
}q("collecting event: "+R);
if(!K[Q]){H=S+H;
U+=S.length;
q("also adding element event: "+S)
}K[Q]=true;
D=F+D
}if(H.length>0){H=H.substring(0,H.length-1)
}if(D.length>0){D=D.substring(0,D.length-1)
}var T=b?1:0;
var E=H+N+D+N+T+ad+X;
return E
}}
})();
